...
4. Prefer the smallest implementation that preserves the user-visible contract. Do not add a service, dependency, polling loop, database write, or message type without a measured need.
5. After network changes, run `npm.cmd run check` and `npm.cmd run build`, then inspect the browser path when available.
6. Never commit secrets, local state, generated package stores, or unrelated dirty-worktree changes.
