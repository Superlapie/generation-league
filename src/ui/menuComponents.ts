...
export function renderFieldMenuRoot(
  ...
  drawPageHeader(scene, { title: 'FIELD MENU', subtitle: opts.location, iconKey: 'ui-icon-card' }, sink);

  // Passport block
  keep(sink, raisedPanel(scene, 6, 24, 72, 110, UI_COLORS.paperCool));
  keep(sink, recessedPanel(scene, 10, 28, 64, 52));
  keep(sink, scene.add.sprite(42, 48, `avatar-${opts.avatar}`, 0).setScale(0.5).setDepth(6));
  keep(sink, addText(scene, 12, 84, opts.playerName.toUpperCase(), 'panelTitle', undefined, 6));
  // Crest sockets
  [0, 1, 2].forEach((i) => {
    const key = i < opts.crests ? 'ui-icon-crest-filled' : 'ui-icon-crest-empty';
    keep(sink, scene.add.image(14 + i * 18, 98, key).setDepth(6));
  });
  keep(sink, addText(scene, 12, 112, `${opts.money} L`, 'numeric', undefined, 6));
  keep(sink, addText(scene, 12, 122, `${opts.guideCaught}/${opts.guideTotal}`, 'compact', undefined, 6));
  keep(sink, drawProgressMeter(scene, 12, 130, 58, opts.guideCaught / Math.max(1, opts.guideTotal), 6));

  // 2-column nav grid
  ROOT_ENTRIES.forEach((entry, index) => {
    const cell = gridCell(ROOT_GRID, index, 4);
    const selected = index === opts.cursor;
    const tile = scene.add.rectangle(cell.x, cell.y, cell.w, cell.h, selectionFill(selected)).setOrigin(0).setDepth(5);
    ...
    if (scene.textures.exists(NAV_ICON_KEYS[index])) {
      keep(sink, scene.add.image(cell.x + 6, cell.y + Math.floor(cell.h / 2), NAV_ICON_KEYS[index]).setDepth(6));
    }
    keep(sink, addText(scene, cell.x + 18, cell.y + Math.floor(cell.h / 2) - 4, entry.label.toUpperCase(), 'menuLabel', selectionTextColor(selected), 6));
  });
...
export function renderItemIcon(scene: Phaser.Scene, item: ItemDefinition, x: number, y: number, sink: UiSink): void {
  if (item.category === 'capture') {
    const img = scene.add.image(x, y, 'capture-pod').setDisplaySize(28, 28).setDepth(6);
  ...
  if (scene.textures.exists(iconKey)) {
    keep(sink, scene.add.image(x, y, iconKey).setDepth(6));
  }
}
...
export function renderBagScreen(
  ...
  POCKET_DEFS.forEach((pocket, index) => {
    ...
    if (scene.textures.exists(pocket.icon)) keep(sink, scene.add.image(x + 4, 27, pocket.icon).setDepth(6));
    keep(sink, addText(scene, x + 16, 27, pocket.label, 'compact', sel ? '#24342d' : '#d6e0cc', 6));
  });
  ...
  if (opts.selectedItem) {
    ...
    keep(sink, scene.add.text(128, 94, opts.selectedItem.description, textStyle('compact')).setDepth(6).setWordWrapWidth(96));
...
export function renderBagTargetScreen(
  ...
  keep(sink, scene.add.text(12, 78, opts.item.description, textStyle('compact')).setDepth(6).setWordWrapWidth(48));
...
export function renderGuideScreen(
  ...
  if (seen) {
    species.types.forEach((type, i) => { if (type) drawTypeBadge(scene, 120 + i * 50, 99, type, typeColor, sink); });
    keep(sink, addText(scene, 120, 114, `${species.height.toFixed(1)}m  ${species.weight.toFixed(1)}kg`, 'compact', undefined, 6));
    keep(sink, addText(scene, 120, 124, `${caught ? 'Captured' : 'Seen'} · ${species.habitat}`, 'compact', caught ? '#8b6b25' : '#52665c', 6));
    keep(sink, scene.add.text(120, 132, species.description, textStyle('compact')).setDepth(6).setWordWrapWidth(108).setMaxLines(3));
  } else {
...
export function renderOptionsScreen(
  ...
  keep(sink, raisedPanel(scene, 10, 28, 220, 108));
  ...
  sections.forEach((sec) => {
    ...
    for (let i = sec.start; i < sec.end && i < opts.rows.length; i++) {
      ...
      drawListRow(scene, { x: 16, y, w: 208, h: 13, label: opts.rows[i], right: opts.values[i], selected }, sink);
      if (i === 2 || i === 3) {
        const vol = i === 2 ? opts.options.musicVolume : opts.options.sfxVolume;
        keep(sink, drawVolumeMeter(scene, 150, y + 3, 8, Math.round(vol * 8), 7));
      }
      if (i === 0 || i === 4 || i === 5) {
        const on = i === 0 ? opts.options.battleScene : i === 4 ? opts.options.muted : opts.options.reducedMotion;
        keep(sink, drawToggleSwitch(scene, 188, y + 2, on, 7));
      }
      y += 14;
    }
    y += 2;
...
export function renderSummaryScreen(
  ...
  if (opts.summaryPage === 0) {
    ...
    keep(sink, scene.add.text(88, 90, species.description, textStyle('compact')).setDepth(6).setWordWrapWidth(130).setMaxLines(4));
  }
...
export function renderSaveModal(scene: Phaser.Scene, opts: { title: string; message: string; success: boolean }, sink: UiSink): void {
  ...
  if (scene.textures.exists(icon)) keep(sink, scene.add.image(56, 72, icon).setDepth(30));
